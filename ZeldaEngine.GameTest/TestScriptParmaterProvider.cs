...
namespace ZeldaEngine.GameTest
{
    public class TestScriptParmaterProvider : IScriptParamaterProvider
    {
        ...
        public TestScriptParmaterProvider(IScriptEngine engine)
        {
        ...
        public bool AddParamater(Type type, IEnumerable<object> @params)
        {
            ...
            //Tell the engine we change the script
            _engine.Report(type, string.Format("Adding params {0} to {1}", string.Join(", ", _paramaters[type.Name].ToArray()), type.Name),
                                  ReportReason.Log);
