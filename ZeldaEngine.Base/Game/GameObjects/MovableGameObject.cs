...
namespace ZeldaEngine.Base.Game.GameObjects
{
    public class MovableGameObject : DrawableGameObject
    {
        ...
        protected override void OnUpdate(float dt)
        {
            ...
            if (Position.X > GameEngine.GameConfig.ScreenWidth)
                Position.X = GameEngine.GameConfig.ScreenWidth;
            if (Position.X < 0)
                Position.X = 0;

            if (Position.Y > GameEngine.GameConfig.ScreenHeight)
                Position.Y = GameEngine.GameConfig.ScreenHeight;
            if (Position.Y < 0)
                Position.Y = 0;
